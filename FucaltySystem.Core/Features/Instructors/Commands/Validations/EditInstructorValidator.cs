...
namespace FacultySystem.Core.Features.Instructors.Commands.Validations
{
    public class EditInstructorValidator : AbstractValidator<EditInstructorCommand>
    {
        ...
        #region ctors
        public EditInstructorValidator(IInstructorService instructorService)
        ...
        {
            RuleFor(x => x.InstructorName).MustAsync(async (Model, Key, CancellationToken) => await _instructorService.IsUnique(Key, Model.InstructorId)).WithMessage("This name is already exist");
        }
        #endregion
    }
}
