...
namespace FacultySystem.Core.Features.Subjects.Commands.Validations
{
    public class EditSubjectValidator : AbstractValidator<EditSubjectCommand>
    {
        ...
        {
            RuleFor(x => x.SubjectName).MustAsync(async (model, Key, CancellationToken) => await _subjectService.IsUnique(Key, model.SubjectId)).WithMessage("This name is already exist");
        }
        #endregion
    }
}
